...
static double I_Y=0, I_P=0;
static double D_Y=0, D_P=0;
...
void isr(){
  ...
  D_Y = (2*kd*(errorY -lastErrorY)+(2*tau-T)*D_Y)/(2*tau+T);
  D_P = (2*kd*(errorP -lastErrorP)+(2*tau-T)*D_P)/(2*tau+T);
  
  I_Y += ki*((lastErrorY + errorY)/2)*T;
  I_P += ki*((lastErrorP + errorP)/2)*T ;

  phiY = kp*errorY + I_Y + D_Y;
  phiP = kp*errorP + I_P + D_P;
